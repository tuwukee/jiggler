  ...
  describe "#push_job" do
    it "pushes an empty job to default queue" do
      expect do
        ...
        config.with_redis(async: false) { |conn| conn.llen("jiggler:list:default") }
      }.by(1)
    end
    ...
    it "pushes job to queue from args" do
      expect do
        ...
        config.with_redis(async: false) { |conn| conn.llen("jiggler:list:mine") }
      }.by(1)
      config.with_redis(async: false) { |conn| conn.del("jiggler:list:mine") }
    end
    ...
    it "does not drop job if queue is not configured" do
      expect do
        ...
        config.with_redis(async: false) { |conn| conn.llen("jiggler:list:unknown") }
      }.by(1)
    end
  end
